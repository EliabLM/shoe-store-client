...
import { ENUM_NAMES, updateCreditSchema } from './newCredit.schema';
...
const UpdateCredit = () => {
  ...
  const { updateCredit } = useCreditorsService();
  ...
  const { handleSubmit, control, setValue } = useForm({
    ...
    resolver: yupResolver(updateCreditSchema),
  });
  ...
  const onSubmit = async (data) => {
    ...
    const { interest_rate, initial_value, creation_date, creditor, active } = newData;
    ...
    setIsLoading(true);
    const resCreateCredit = await updateCredit({ body });
  ...
  useEffect(() => {
    if (!state) return navigate('/acreedores/creditos/lista-creditos');
    if (!data) return;

    const creditor = options.filter((item) => item.value === state.creditor._id)?.[0];

    setValue(ENUM_NAMES.active, state.active);
    setValue(ENUM_NAMES.creation_date, state.creationDate);
    setValue(ENUM_NAMES.creditor, creditor);
    setValue(ENUM_NAMES.initial_value, state.initialValue);
    setValue(ENUM_NAMES.interest_rate, `${state.interestRate}%`);
  }, [state, options]);
